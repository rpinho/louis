"""
T-Cell Target Explorer — .mcpb bundle entry point (Claude Desktop).

Runs the MCP server over stdio. Self-contained: bundled deps in lib/, dataset in
data/, engine in tcell_targets/. The knowledge base is copied to a user-writable dir
...
sys.path.insert(0, str(HERE))           # the tcell_targets package

# Make the KB writable: seed a user dir from the bundled kb/ once, point the KB at it.
kb_target = Path(os.environ.get("TCELL_KB_DIR") or (Path.home() / ".tcell-target-explorer" / "kb"))
bundled_kb = HERE / "kb"
...
os.environ["TCELL_KB_DIR"] = str(kb_target)

from tcell_targets.mcp_server import main
